...
def index(request):
    ...
    proximos_eventos = PostsModel.objects.filter(event_date__gte=hoy).order_by(
        "event_date"
    )[:6]

    localidades = (
        PostsModel.objects.filter(localidad__isnull=False)
...
def busqueda_views(request):
    ...
    resultados = PostsModel.objects.filter(event_date__gte=hoy)
    ...
    if fecha:
        resultados = resultados.filter(event_date=fecha)

    localidades = PostsModel.objects.values_list("localidad", flat=True).distinct()

    contexto = {
        "proximos_eventos": resultados.order_by("event_date"),
        "query": query,
        "localidad": localidad,
        "fecha": fecha,
        "localidades": localidades,
